        ...
        <el-table v-loading="listLoading" :data="dataList" element-loading-text="正在查询中。。。" border fit highlight-current-row row-key="id">
            ...
            <el-table-column align="center" label="操作" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    ...
                    <el-button type="danger" size="mini" @click="handleBack()">返回模板</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
import { listPara, createPara, updatePara, deletePara } from '@/api/para'
...
export default {
  ...
  methods: {
    getDataList() {
      ...
      listPara(this.listQuery).then(response => {
    ...
    handleCreate() {
      this.$router.push({ path: '/template/paraEdit' })
    },
    handleUpdate(row) {
      this.$router.push({ path: '/template/paraEdit', query: { paraId: row.id }})
    },
    handleDelete(row) {
      deletePara(row)
      .then(response => {
